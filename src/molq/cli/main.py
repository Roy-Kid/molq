...
def create_submitter(scheduler: str, **kwargs):
    ...
    if scheduler == 'local':
        ...
        username = kwargs.get('username') or molq_config.get('slurm.username', os.getenv('USER'))
...
def submit(ctx, scheduler, command_args, **kwargs):
    ...
    if cmd:
        ...
        click.echo("Error: No command provided. Use arguments or pipe script via stdin.", err=True)
        sys.exit(1)
...
@click.option('--scheduler', type=click.Choice(['local', 'slurm']), help='Filter by scheduler')
@click.option('--all-history', is_flag=True, help='Show all jobs including finished/cancelled')
...
@click.option('--scheduler', type=click.Choice(['local', 'slurm']), help='Scheduler to query')
def status(job_id, scheduler):
    ...
    if not scheduler:
        if '-' in job_id:
            ...
            click.echo("Error: Please specify --scheduler or use format 'scheduler-jobid'", err=True)
            sys.exit(1)
...
@cli.command()
@click.argument('job_id')
@click.option('--scheduler', type=click.Choice(['local', 'slurm']), help='Scheduler to cancel from')
