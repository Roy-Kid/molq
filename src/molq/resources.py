...
class BaseResourceSpec(BaseModel):
    ...
    output_file: Optional[str] = Field(
        None,
        description="Path to standard output file"
    )
    
    error_file: Optional[str] = Field(
        None,
        description="Path to standard error file"
    )
...
class ComputeResourceSpec(BaseResourceSpec):
    ...
    memory: Optional[str] = Field(
        None,
        description="Total memory requirement (e.g., '8GB', '512MB')"
    )
    
    # Time management
    time_limit: Optional[str] = Field(
        None,
        description="Maximum runtime (e.g., '2h30m', '02:30:00', '1d')"
    )
...
class ClusterResourceSpec(ComputeResourceSpec):
    ...
    queue: Optional[str] = Field(
        None,
        description="Target queue/partition for job execution",
        alias="partition"
    )
    ...
    cpu_per_node: Optional[int] = Field(
        None,
        description="CPU cores per compute node",
        gt=0
    )
    
    memory_per_cpu: Optional[str] = Field(
        None,
        description="Memory per CPU core (e.g., '2GB', '4GB')"
    )
    ...
    gpu_type: Optional[str] = Field(
        None,
        description="Specific GPU type (e.g., 'v100', 'a100', 'rtx3090')"
    )
    ...
    array_spec: Optional[str] = Field(
        None,
        description="Array job specification (e.g., '1-100', '1-100:5')"
    )
    ...
    comment: Optional[str] = Field(
        None,
        description="Job description/comment"
    )
    
    # Advanced options
    constraints: Optional[Union[str, List[str]]] = Field(
        None,
        description="Node feature constraints"
    )
    
    licenses: Optional[Union[str, List[str]]] = Field(
        None,
        description="Software licenses required"
    )
    
    reservation: Optional[str] = Field(
        None,
        description="Use specific reservation"
    )
    ...
    def map_resources(self, spec: ResourceSpec) -> Dict[str, str]:
        ...
        for attr, slurm_param in self.PARAMETER_MAPPING.items():
            ...
            if attr == 'time_limit':
                ...
                elif isinstance(value, str) and value in [p.value for p in PriorityLevel]:
...
class ResourceManager:
    ...
    @classmethod
    def create_spec(cls, spec_type: str = "cluster", **kwargs) -> Union[BaseResourceSpec, ComputeResourceSpec, ClusterResourceSpec]:
        ...
        if spec_type == "base":
            ...
            raise ValueError(f"Unknown spec type: {spec_type}. Use 'base', 'compute', or 'cluster'.")
    ...
    @classmethod
    def map_to_scheduler(cls, spec: ResourceSpec, scheduler_type: str) -> Dict[str, str]:
