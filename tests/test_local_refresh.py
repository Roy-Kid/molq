...
def test_local_refresh_job_status(tmp_molq_home):
    ...
    job_id = proc.pid
    s.register_job("local", job_id, "test_sleep", JobStatus.Status.RUNNING, "sleep 2", "/tmp", time.time(), {"cmd": ["sleep", "2"]})
...
def test_local_refresh_job_status_zombie(tmp_molq_home):
    ...
    fake_pid = 999999
    s.register_job("local", fake_pid, "fake_job", JobStatus.Status.RUNNING, "echo hi", "/tmp", time.time(), {"cmd": ["echo", "hi"]})
    # Refresh all jobs
    s.refresh_all_jobs("local")
