...
class DummySubmitor(BaseSubmitor):
    def local_submit(self, job_name: str, cmd: str | list[str], cwd=None, block: bool = False, **kwargs):
        return 12345
    def remote_submit(self, job_name: str, cmd: str | list[str], cwd=None, block: bool = False, **kwargs):
        return 54321
...
def test_job_db_isolated(tmp_molq_home):
    ...
    s._init_job_db()
    # Register job
    s.register_job("local", 1, "job1", JobStatus.Status.RUNNING, "echo 1", "/tmp", 1234567890, {"foo": "bar"})
    s.register_job("slurm@mock", 2, "job2", JobStatus.Status.PENDING, "echo 2", "/tmp", 1234567890, {"bar": "baz"})
    # List jobs
...
def test_list_jobs_multi_section(tmp_molq_home):
    ...
    s = DummySubmitor("local")
    s._init_job_db()
    s.register_job("local", 1, "job1", JobStatus.Status.RUNNING, "echo 1", "/tmp", 1234567890)
    s.register_job("slurm@mock", 2, "job2", JobStatus.Status.RUNNING, "echo 2", "/tmp", 1234567890)
