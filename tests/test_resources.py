...
from molq.resources import (
    BaseResourceSpec, ComputeResourceSpec, ClusterResourceSpec,
    ResourceManager, TimeParser, MemoryParser,
    SlurmMapper, PbsMapper, LsfMapper,
    PriorityLevel, EmailEvent,
    create_compute_job, create_gpu_job, create_array_job, create_high_memory_job
)
...
class TestBaseResourceSpec:
    ...
    def test_alias_support(self):
        ...
        spec = BaseResourceSpec(
            cmd="python test.py",
            cwd="/home/user"  # Using alias instead of workdir
        )
...
class TestClusterResourceSpec:
    ...
    def test_gpu_validation(self):
        ...
        with pytest.raises(ValueError):
            ClusterResourceSpec(
                cmd="python test.py",
                gpu_type="v100"  # Missing gpu_count
            )
